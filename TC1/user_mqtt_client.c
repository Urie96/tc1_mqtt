    ...
    if ( mico_rtos_is_queue_empty( &mqtt_msg_send_queue ) == true )
    {
        ...
        switch ( timer_status )
        {
            ...
            case 10:
                ...
                break;
            default:
                mico_rtos_stop_timer( &timer_handle );
...
void mqtt_client_thread( mico_thread_arg_t arg )
{
    ...
    while ( 1 )
    {
        ...
        {
            mqtt_log("ERROR: MQTT server config invalid (ip=%s, port=%d), using hardcoded default.", MQTT_SERVER, MQTT_SERVER_PORT);
    ...
    isconnect = true;
    timer_status = 0;
    mico_rtos_init_timer( &timer_handle, 150, user_mqtt_timer_func, &arg );
    mico_rtos_start_timer( &timer_handle );
